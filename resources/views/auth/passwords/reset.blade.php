                    ...
                    <form class="py-10 px-5" method="POST" action="{{ route('password.update') }}" novalidate>
                        ...
                        <div class="flex flex-wrap mb-6">
                            <label for="email" class="block text-gray-700 text-sm mb-2">{{ __('E-Mail Address') }}</label>
                                ...
                                <input id="email" type="email" class="p-3 bg-gray-100 rounded form-input w-full border-gray-600 border @error('email') border-red-500 border @enderror" name="email" value="{{ $email ?? old('email') }}" autocomplete="email" autofocus>
