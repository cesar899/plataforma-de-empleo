...
@section('content')
 <h1 class="text-2xl text-center mt-10">editando vacante {{ $vacancie->title }}</h1>
  <form class="max-w-lg mx-auto my-10" action="{{ route('vacantes.update',$vacancie->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="mb-5">
         ...
         border-gray-600 border @error('title') border-red-500 border @enderror" placeholder="titulo de la vacante" name="title" value="{{ $vacancie->title}}">
      ...
      <div class="mb-5">
         ...
         <select id="categorie" class="rounded appearance-none w-full border border-gray-600 border text-gray-700 rounend leading-light focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100" name="categorie">
            ...
            @foreach($categories as $item)
               <option {{ $vacancie->categorie_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">
                  {{$item->name}}
               </option>
            @endforeach   
         </select>
      ...
      <div class="mb-5">
         ...
         <select id="experience" class="block appearance-none w-full border border-gray-600 border text-gray-700 rounded leading-light focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100" name="experience">
            ...
            @foreach($experiences as $item)
               <option {{ $vacancie->experience_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">
                  {{$item->name}}
               </option>
            @endforeach   
         </select>
      ...
      <div class="mb-5">
         ...
         <select id="ubication" class="block appearance-none w-full border border-gray-600 border text-gray-700 rounded leading-light focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100" name="ubication">
            ...
            @foreach($ubications as $item)
               <option {{ $vacancie->ubication_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">
                  {{$item->name}}
               </option>
            @endforeach   
         </select>
      ...
      <div class="mb-5">
         ...
         <select id="salary" class="block appearance-none w-full border border-gray-600 border text-gray-700 rounded leading-light focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-gray-100" name="salary">
            ...
            @foreach($salary as $item)
               <option {{ $vacancie->salary_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">
                  {{$item->name}}
               </option>
            @endforeach   
         </select>
      ...
      <div class="mb-5">
         ...
         <list-skills
          ...
          :oldskills="{{ json_encode( $vacancie->skills ) }}"
          ></list-skills>
         @error('skills')
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mt-3 mb-6" role="alert">
              <strong class="font-bold">Error!</strong>
      ...
      <button type="submit"
         class="bg-green-500 w-full hover:bg-green-700 text-gray-100 p-3 focus:outline focus:shadow-outline uppercase font-bold ">
         Publicar Vacante
      </button>     
   </form>
